...
class Factory:
    ...
    def create_session_from_configuration(communicator_configuration,
                                          ...
                                          authenticator=None):
        """
        Creates a Session based on the configuration stored in the CommunicatorConfiguration argument
        """
        # If an authenticator is not given, api_key_id and secret_api_key are used to create a DefaultAuthenticator used to create the session.
        if meta_data_provider is None:
            meta_data_provider = MetaDataProvider(integrator=communicator_configuration.integrator,
        ...
        if authenticator is None:
            authenticator = DefaultAuthenticator(communicator_configuration.authorization_type,
    ...
    @staticmethod
    def create_session_from_file(configuration_file_name, api_key_id, secret_api_key,
                                 meta_data_provider=None,
                                 connection=None,
                                 authenticator=None):
        """
        Creates a Session based on the configuration values in configuration_file_name, api_id_key and secret_api_key.
        """
        configuration = Factory.create_configuration(configuration_file_name, api_key_id, secret_api_key)
        return Factory.create_session_from_configuration(configuration,
                                                         meta_data_provider=meta_data_provider,
                                                         connection=connection,
                                                         authenticator=authenticator)
    ...
    def create_communicator_from_session(session):
        """
        Create a Communicator based on the settings stored in the Session argument
        """
    ...
    def create_communicator_from_configuration(communicator_configuration):
        """
        Create a Communicator based on the configuration stored in the CommunicatorConfiguration argument
        """
        session = Factory.create_session_from_configuration(communicator_configuration)
        return Communicator(session, DefaultMarshaller.INSTANCE())

    @staticmethod
    def create_communicator_from_file(configuration_file_name, api_key_id, secret_api_key):
        """
        Creates a Communicator based on the configuration values in configuration_file_name, api_key_id and secret_api_key.
        """
        configuration = Factory.create_configuration(configuration_file_name, api_key_id, secret_api_key)
    ...
    @staticmethod
    def create_client_from_file(configuration_file_name, api_key_id, secret_api_key):
        """
