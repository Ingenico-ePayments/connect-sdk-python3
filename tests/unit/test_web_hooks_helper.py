...
from ingenico.connect.sdk.webhooks.secret_key_not_available_exception import \
...
class WebhooksHelperTest(unittest.TestCase):
    ...
    def test_unmarshal_missing_headers(self):
        ...
        InMemorySecretKeyStore.INSTANCE().store_secret_key(self.__KEY_ID,
                                                           self.__SECRET_KEY)
        body = self.__read_resource("valid-body")
        self.assertRaises(SignatureValidationException,
    ...
    def test_unmarshal_string_success(self):
        ...
        self.assertIsNone(
            event.payment.payment_output.direct_debit_payment_method_specific_output)
        self.assertIsNone(
            event.payment.payment_output.invoice_payment_method_specific_output)
        self.assertIsNone(
            event.payment.payment_output.redirect_payment_method_specific_output)
        self.assertIsNone(
            event.payment.payment_output.sepa_direct_debit_payment_method_specific_output)
        self.assertIsNotNone(
            event.payment.payment_output.bank_transfer_payment_method_specific_output)
        self.assertEqual(11,
                         event.payment.payment_output.bank_transfer_payment_method_specific_output.payment_product_id)
        ...
        self.assertEqual("20170202112414",
    ...
    def __read_resource(self, resource):
        output = file_utils.read_file(
            os.path.join("webhooks", resource))
    ...
    def test_are_equal_signatures_equal(self):
        ...
        self.assertTrue(
            WebhooksHelper.are_equal_signatures(signature, signature))
    ...
    def test_are_equal_signatures_not_equal(self):
        ...
        self.assertFalse(
            WebhooksHelper.are_equal_signatures(signature, signature + "1"))
        self.assertFalse(
            WebhooksHelper.are_equal_signatures(signature + "1", signature))
        self.assertFalse(WebhooksHelper.are_equal_signatures(signature.lower(),
                                                             signature.upper()))
        self.assertFalse(WebhooksHelper.are_equal_signatures(signature.upper(),
                                                             signature.lower()))
        ...
        self.assertFalse(
            WebhooksHelper.are_equal_signatures(large + "1", large))
        self.assertFalse(
            WebhooksHelper.are_equal_signatures(large, large + "1"))
        self.assertFalse(
            WebhooksHelper.are_equal_signatures(large.lower(), large.upper()))
        self.assertFalse(
            WebhooksHelper.are_equal_signatures(large.upper(), large.lower()))
